            ...
            <div id="Schatten"class="panel-body">
                ...
                @if(\Illuminate\Support\Facades\Auth::user()->hasRole('School'))
                    @if(\Illuminate\Support\Facades\Auth::user()->School()->first()->teachID == \Illuminate\Support\Facades\Auth::user()->teachID)
                        <button title="@lang('tags.school_app_menu_school_title')" style="width:100%" onclick="location.href ='{{route('school.home')}}';" type="button" class="Layoutbuttons btn btn-default btn-lg btn-block">@lang('profiles.teacher_lnt_school')</button>
                    @endif
                @endif
