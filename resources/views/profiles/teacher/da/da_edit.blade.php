        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        {{csrf_field()}}
                        {!! Form::open(array('action' => 'Profile\DA\TeacherDAController@settingsupdate', 'method'=>'POST')) !!}
                        ...
                        {!! Form::select('emp', $tchs,$da->emplID, ['class'=>'form-control', 'required']) !!}
                        <!---@if(count($da->employee()->get()) <1)
                            <p>@lang('form.company_da_edit_form_noempl')</p>
                            @else
                            <p>@lang('form.company_da_edit_form_current') {{$da->empteacher()->first()->name}}</p>
                            @endif --->
                        </div>
                        <div class="form-group">
                        {!! Form::label('empz', __('form.company_da_edit_form_emplz')) !!}
                        {!! Form::select('empz', $tchs,$da->emplIDz, ['class'=>'form-control']) !!}
                        <!---@if(count($da->employeez()->get()) <1)
                            <p>@lang('form.company_da_edit_form_noemplz')</p>
                            @else
                            <p>@lang('form.company_da_edit_form_current') {{$da->empzteacher()->first()->name}}</p>
                            @endif--->
                        </div>
                        <div class="form-group">
